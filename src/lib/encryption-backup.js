// https://github.com/nodejs/node/issues/21553
// TODO:
// - crypto_stream_chacha20 missing!
// - crypto_stream_chacha20_xor_ic

// import { api as Sodium } from 'sodium';
// import { api as Sodium } from 'sodium-universal';
...
function computePoly1305(cipherText, AAD, nonce, key) {
    ...
    // Original: const computed_hmac = Sodium.crypto_onetimeauth(msg, polyKey);
    const computed_hmac = new Buffer(Sodium.crypto_onetimauth_BYTES);
    Sodium.crypto_onetimeauth(computed_hmac, msg, polyKey);
...
function verifyAndDecrypt(cipherText, mac, AAD, nonce, key) {
    const matches = Sodium.crypto_verify_16(mac, computePoly1305(cipherText, AAD, nonce, key));
...
function encryptAndSeal(plainText, AAD, nonce, key) {
    const cipherText = Sodium.crypto_stream_chacha20_xor_ic(plainText, nonce, 1, key);
